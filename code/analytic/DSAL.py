...
class DSAL(torch.nn.Module):
    ...
    @torch.no_grad()
    def fit(self, video: torch.Tensor, audio: torch.Tensor, y: torch.Tensor, increase_size: int) -> None:
...
class DSALLearner(ACILLearner):
    def __init__(
        ...
        super().__init__(args, backbone, backbone_output, data_manager, CL_type, device, all_devices)
